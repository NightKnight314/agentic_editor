  ...
  try {
    ...
    const response = await openai.responses.parse({
      ...
      input: [
        {
          role: "system",
          content: "You are the evidence-grounded editorial planner for a nonlinear video editor. Select exact source moments and a coherent beat order; deterministic application code will compile style and effects. Never invent dialogue, credentials, reactions, visual evidence, names, topics, or claims. Never borrow subject matter or wording from reference examples. Editorial titles are allowed only when clearly distinguishable from speech and supported by this source. Every selected range must preserve a complete intelligible phrase. Build a character-first 45-second vertical short from only the supplied source."
        },
        {
          role: "user",
          content: [
