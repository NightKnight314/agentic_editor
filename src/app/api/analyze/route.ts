  ...
  try {
    ...
    const response = await openai.responses.parse({
      ...
      input: [
        {
          role: "system",
          content: "You are the editorial brain of a nonlinear video editor. Analyze source events and return a concrete source-to-timeline edit decision list. Think in timestamped clips, tracks, titles, transitions, and bounded effect stacks. Never invent spoken dialogue. Editorial titles are allowed but must not be presented as quotes. Every source range must be supported by the transcript or sampled visuals. The requested output is a coherent 30-60 second vertical short, not a summary of the whole source."
        },
        {
          role: "user",
          content: [
            {
              type: "input_text",
              text: `SOURCE\nName: ${sourceName}\nDuration: ${sourceDuration.toFixed(2)} seconds\n\nUSER BRIEF\n${brief}\n\nSTYLE MAP\n${JSON.stringify({ creativeThesis: kumarStyle.creativeThesis, storyBeats: kumarStyle.storyBeats, pacing: kumarStyle.pacing, plannerRules: kumarStyle.plannerRules, review: kumarStyle.review })}\n\nTIMESTAMPED TRANSCRIPT\n${transcriptForPlanner}\n\nReturn 8-36 useful source events and a 6-10 segment edit plan. Every selected segment must be 2-8 seconds long and use complete, intelligible speech. Calculate the sum of all sourceEnd-sourceStart durations before responding; it must be 35-50 seconds and match targetDuration. Preserve the ending beats instead of spending the duration budget on early exposition. Prefer a strong declaration in the first 3 output seconds, escalate through proof, include a candid humanizing beat when supported, and end cleanly. Reordering is allowed when it improves the story.`
            },
            ...visualContent
          ]
        }
